...
for iter in tqdm(range(total_iterations), bar_format='[{elapsed}] {n_fmt}/{total_fmt} | {l_bar}{bar} {rate_fmt}{postfix}'):
    ...
    with open(file_path, 'w') as file:
        file.write(f"PENN_ZI = c({", ".join(str(item) for item in PENN_MI_loss)}) \n")
        file.write('\n')
        file.write(f"NN_ZI = c({", ".join(str(item) for item in NN_MI_loss)}) \n")
        file.write('\n')
        file.write(f"PENN_MF = c({", ".join(str(item) for item in PENN_MF_loss)}) \n")
        file.write('\n')
        file.write(f"NN_MF = c({", ".join(str(item) for item in NN_MF_loss)}) \n")
        file.write('\n')
        file.write(f"PENN_MICE = c({", ".join(str(item) for item in PENN_II_loss)}) \n")
        file.write('\n')
        file.write(f"NN_MICE = c({", ".join(str(item) for item in NN_II_loss)}) \n")