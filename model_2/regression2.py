...
width_1 = ceil(np.sqrt(0.5*n/2))   # note that training sample size is 0.5n
...
def one_run():
    ...
    def reg_func(x):
        y = 2*np.sin(2*x[1] + 2*x[2] + 2*x[3]) + 2*x[3]    # Bayes risk approx 0.6407 + sigma^2
        return y
    ...
    # Generate Omega, which depends on X, so MNAR
    ...
    Z_nan_train = Z_nan[0:round(n/2), :]
    Z_nan_val = Z_nan[round(n/2):round(3*n/4), :]
    Z_nan_test = Z_nan[round(3*n/4):n, :]
    ...
    for max_depth in [3, 6, 9, 12]:
        ...
        model.fit(Z_nan_train, Y_train.detach().cpu().numpy().ravel())
        xgb_val_mse = np.mean((Y_val.detach().cpu().numpy().ravel() - model.predict(Z_nan_val))**2)
        if xgb_val_mse < best_xgb_val_mse:
            best_xgb_val_mse = xgb_val_mse
            xgb_test_mse = np.mean((Y_test.detach().cpu().numpy().ravel() - model.predict(Z_nan_test))**2)                                           
                                               
    return PENN_MI_loss.item(), NN_MI_loss.item(), PENN_MF_loss.item(), NN_MF_loss.item(), PENN_II_loss.item(), NN_II_loss.item(), xgb_test_mse
...
def one_run_rf():
    ...
    # Define regression function
    def reg_func(x):
        y = 2*np.sin(2*x[1] + 2*x[2] + 2*x[3]) + 2*x[3]    
        return y
    ...
    for max_features in [ceil(0.1*d), ceil(0.2*d), ceil(0.4*d), ceil(0.8*d)]:
        ...
        model.fit(Z_nan_train, Y_train)
        rf_val_mse = np.mean((Y_val - model.predict(Z_nan_val))**2)
        if rf_val_mse < best_rf_val_mse:
            best_rf_val_mse = rf_val_mse
            rf_test_mse = np.mean((Y_test - model.predict(Z_nan_test))**2)                                           
