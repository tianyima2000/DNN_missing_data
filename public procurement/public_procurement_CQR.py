...
def CQR(quantiles_pred, y_true, alpha):
    with torch.no_grad():
        quantiles_pred = quantiles_pred.cpu().numpy()
...
def train_test_model(model, Z_train, Z_val, Z_cal, Z_test, Y_train, Y_val, Y_cal, Y_test, lr, prune_amount, Omega_train=None, Omega_val=None, Omega_cal=None, Omega_test=None, epochs=200, weight_decay=0.001, prune_start=10, patience=10, live_plot=False):
    ...
    if Omega_train is not None:
        ...
        model.eval()
        with torch.no_grad():
            pred_intervals = model(Z_test, Omega_test).cpu().numpy()
    ...
    if Omega_train is None:
        ...
        model.eval()
        with torch.no_grad():
            pred_intervals = model(Z_test).cpu().numpy()
...
for iter in tqdm(range(total_iterations), bar_format='[{elapsed}] {n_fmt}/{total_fmt} | {l_bar}{bar} {rate_fmt}{postfix}'):
    ...
    Z_MI_cal = torch.tensor(Z_MI_cal.to_numpy(), dtype=torch.float32).to(device)
    ...
    Z_MF_cal = torch.tensor(Z_MF_cal.to_numpy(), dtype=torch.float32).to(device)
    ...
    Z_II_cal = torch.tensor(Z_II_cal.to_numpy(), dtype=torch.float32).to(device)
    ...
    Omega_cal = torch.tensor(Omega_cal.to_numpy(), dtype=torch.float32).to(device)
    ...
    Y_cal = torch.tensor(Y_cal.to_numpy().reshape(-1,1), dtype=torch.float32).to(device)
    ...
    file_path = os.path.join(script_dir, "output_CQR.txt")
