...
for iter in tqdm(range(total_iterations), bar_format='[{elapsed}] {n_fmt}/{total_fmt} | {l_bar}{bar} {rate_fmt}{postfix}'):
    ...
    def reg_func(x):
        y = np.sin(2 * x[1] + 1) + np.abs(x[2]) * (x[3]**2 + 1) / 3    # Bayes risk approx 0.7075 + sigma^2
        return y
    ...
    prune_amount_vec = [0.9, 0.85, 0.8, 0.7, 0.5]

    PENN_ZI_loss[iter] = train_test_best_model(model_class=PENN, Z=Z_ZI, Omega=Omega, Y=Y, lr=0.001, prune_amount_vec=prune_amount_vec) - 0.7075 - sigma**2
    NN_ZI_loss[iter] = train_test_best_model(model_class=NN, Z=Z_ZI, Y=Y, lr=0.001, prune_amount_vec=prune_amount_vec) - 0.7075 - sigma**2

    PENN_MF_loss[iter] = train_test_best_model(model_class=PENN, Z=Z_MF, Omega=Omega, Y=Y, lr=0.001, prune_amount_vec=prune_amount_vec) - 0.7075 - sigma**2
    NN_MF_loss[iter] = train_test_best_model(model_class=NN, Z=Z_MF, Y=Y, lr=0.001, prune_amount_vec=prune_amount_vec) - 0.7075 - sigma**2

    PENN_MICE_loss[iter] = train_test_best_model(model_class=PENN, Z=Z_MICE, Omega=Omega, Y=Y, lr=0.001, prune_amount_vec=prune_amount_vec) - 0.7075 - sigma**2
    NN_MICE_loss[iter] = train_test_best_model(model_class=NN, Z=Z_MICE, Y=Y, lr=0.001, prune_amount_vec=prune_amount_vec) - 0.7075 - sigma**2
