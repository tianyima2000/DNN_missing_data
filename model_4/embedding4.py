...
def prune_and_reinit(model, amount, init_fn=nn.init.kaiming_uniform_):
    ...
    Hence f2, the pattern embedding function, is not pruned.
    """
...
def reg_func(x):
    """
    Regression function from model_8.
    """
...
def generate_and_impute_data(random_state=1):
    """
    Generate the synthetic model_8 data.

    This model combines:
      - dependent covariates X[:, 1] and X[:, 3];
      - sinusoidal regression function;
      - MNAR missingness through Omega[:, 2] and Omega[:, 3].
    """
    ...
    # MNAR missingness for variables 2 and 3
    Omega[:, 2] = (X[:, 2] <= 0.4)
    Omega[:, 3] = (X[:, 3] <= 0.4)
